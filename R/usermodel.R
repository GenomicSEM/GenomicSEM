    ...
    std.lv = FALSE, imp_cov = FALSE, fix_resid = TRUE, toler = NULL) 
{
    ...
    y <- y[!duplicated(apply(y, 1, function(x) paste(sort(x), 
        collapse = ""))), ]
    ...
    for (i in 1:length(S_names)) {
        ...
        if (b == FALSE) {
            remove <- paste0("\\b", colnames(S_LD)[i], "\\b", 
                sep = "")
            remove2[w] <- i
            V_LD <- V_LD[-grep(pattern = remove[1], row.names(V_LD)), 
                -grep(pattern = remove[1], colnames(V_LD))]
            w <- w + 1
            if (!(is.matrix(V_LD))) {
    ...
    z <- (k * (k + 1))/2
    S_LDb <- S_LD
    smooth1 <- ifelse(eigen(S_LD)$values[nrow(S_LD)] <= 0, S_LD <- as.matrix((nearPD(S_LD, 
        corr = FALSE))$mat), S_LD <- S_LD)
    LD_sdiff <- max(abs(S_LD - S_LDb))
    V_LDb <- V_LD
    smooth2 <- ifelse(eigen(V_LD)$values[nrow(V_LD)] <= 0, V_LD <- as.matrix((nearPD(V_LD, 
        corr = FALSE))$mat), V_LD <- V_LD)
    ...
    if (CFIcalc == TRUE) {
        write.null <- function(k, label = "V", label2 = "VF") {
            Model3 <- ""
            for (p in 1:k) {
                linestart3 <- paste(colnames(S_LD)[p], " ~~ ", 
                  colnames(S_LD)[p], sep = "")
                Model3 <- paste(Model3, linestart3, " \n ", sep = "")
            }
            Model2 <- ""
            for (p in 1:k) {
                linestart2 <- paste(label2, p, " =~ 1*", colnames(S_LD)[p], 
                  sep = "")
                Model2 <- paste(Model2, linestart2, " \n ", sep = "")
            }
            Modelsat <- ""
            for (i in 1:(k - 1)) {
                linestartc <- paste(colnames(S_LD)[i], " ~~ 0*", 
                  colnames(S_LD)[i + 1], sep = "")
                if (k - i >= 2) {
                  linemidc <- ""
                  for (j in (i + 2):k) {
                    linemidc <- paste(linemidc, " + 0*", colnames(S_LD)[j], 
                      sep = "")
                  }
                }
                else {
            ...
            ModelsatF <- ""
            for (i in 1:(k - 1)) {
                linestartc <- paste(" ", label2, i, " ~~ 0*", 
                  label2, i + 1, sep = "")
                if (k - i >= 2) {
                  linemidc <- ""
                  for (j in (i + 2):k) {
                    linemidc <- paste(linemidc, " + 0*", label2, 
                      j, sep = "")
                  }
                }
                else {
            ...
            Model4 <- ""
            for (p in 1:k) {
                linestart4 <- paste(label2, p, " ~~ 0*", label2, 
                  p, sep = "")
                Model4 <- paste(Model4, linestart4, " \n ", sep = "")
            }
            modelCFI <- paste(Model3, Model2, ModelsatF, Modelsat, 
                Model4)
            return(modelCFI)
        }
    ...
    if (estimation == "ML") {
        empty4 <- .tryCatch.W.E(Model1_Results <- sem(Model1, 
            ...
            std.lv = std.lv, optim.dx.tol = 0.01, sample.cov.rescale = FALSE, optim.force.converged = T))
    }
    ...
    if (fix_resid == TRUE) {
        if (class(empty4$value)[1] == "simpleError" | lavInspect(Model1_Results, 
            ...
            Model3 <- ""
            for (p in 1:k) {
                linestart3a <- paste(colnames(S_LD)[p], " ~~ ", 
                  paste(n[, p], collapse = ""), "*", colnames(S_LD)[p], 
                  sep = "")
                linestart3b <- paste(paste(n[, p], collapse = ""), 
                  " > .0001", sep = "")
                Model3 <- paste(Model3, " \n ", linestart3a, 
                  " \n ", linestart3b, " \n ", sep = "")
            }
            ...
            if (class(empty4$value)[1] != "lavaan") {
                if (grepl("duplicate", as.character(empty4$value)[1]) == 
                  ...
                  print("The model as initially specified failed to converge. A lower bound of 0 on residual variances was automatically added to try and troubleshoot this. This behavior can be toggled off by setting the fix_resid argument to FALSE.")
                }
            }
            else {
                print("The model as initially specified failed to converge. A lower bound of 0 on residual variances was automatically added to try and troubleshoot this. This behavior can be toggled off by setting the fix_resid argument to FALSE.")
    ...
    bread2 <- .tryCatch.W.E(bread <- solve(t(S2.delt) %*% S2.W %*% 
        S2.delt, tol = toler))
    ...
    if (class(bread2$value)[1] == "matrix") {
        ...
        if (r > 1) {
            empty <- .tryCatch.W.E(check <- lowerTriangle(lavInspect(Model1_Results, 
                "cor.lv")[1:r, 1:r]))
            t <- max(check)
            t2 <- min(check)
        }
        ...
        if (t > 1 | t2 < -1 | t == "NaN") {
            ...
            if (CFIcalc == TRUE) {
                ...
                if (estimation == "ML") {
                  testCFI <- .tryCatch.W.E(fitCFI <- sem(modelCFI, 
                    sample.cov = S_LD, estimator = "ML", sample.nobs = 200, 
                    optim.dx.tol = 0.01, sample.cov.rescale = FALSE, optim.force.converged = F))
                }
                testCFI$warning$message[1] <- ifelse(is.null(testCFI$warning$message), 
                  testCFI$warning$message[1] <- "Safe", testCFI$warning$message[1])
                testCFI$warning$message[1] <- ifelse(is.na(inspect(fitCFI, 
                  "se")$theta[1, 2]) == TRUE, testCFI$warning$message[1] <- "lavaan WARNING: model has NOT converged!", 
                  testCFI$warning$message[1])
                if (as.character(testCFI$warning$message)[1] != 
                  ...
                  if (estimation == "ML") {
                    testCFI2 <- .tryCatch.W.E(ModelQ_Results_CFI <- sem(model = ModelQ_CFI, 
                      sample.cov = S_LD, estimator = "ML", sample.nobs = 200, 
                      optim.dx.tol = 0.01, sample.cov.rescale = FALSE, optim.force.converged = T))
                  }
                  testCFI2$warning$message[1] <- ifelse(is.null(testCFI2$warning$message), 
                    testCFI2$warning$message[1] <- "Safe", testCFI2$warning$message[1])
                  testCFI2$warning$message[1] <- ifelse(is.na(inspect(ModelQ_Results_CFI, 
                    "se")$theta[1, 2]) == TRUE, testCFI2$warning$message[1] <- "lavaan WARNING: model has NOT converged!", 
                    testCFI2$warning$message[1])
                  if (as.character(testCFI2$warning$message)[1] != 
                    "lavaan WARNING: model has NOT converged!") {
                    S2.delt_Q_CFI <- lavInspect(ModelQ_Results_CFI, 
                      "delta")
                    S2.W_Q_CFI <- lavInspect(ModelQ_Results_CFI, 
                      "WLS.V")
                    bread_Q_CFI <- solve(t(S2.delt_Q_CFI) %*% 
                      S2.W_Q_CFI %*% S2.delt_Q_CFI)
                    lettuce_Q_CFI <- S2.W_Q_CFI %*% S2.delt_Q_CFI
                    Ohtt_Q_CFI <- bread_Q_CFI %*% t(lettuce_Q_CFI) %*% 
                      V_Reorder2 %*% lettuce_Q_CFI %*% bread_Q_CFI
                    V_etaCFI <- Ohtt_Q_CFI
                    ...
                    eta_test_CFI <- subset(eta_test_CFI, eta_test_CFI$free != 
                      0)
                    eta_CFI <- cbind(eta_test_CFI[, 14])
                    Q_CFI <- t(eta_CFI) %*% P1_CFI %*% solve(Eig_CFI) %*% 
                      t(P1_CFI) %*% eta_CFI
                  }
                  else {
                ...
                if (!(is.character(Q_CFI)) & !(is.character(Q))) {
                  CFI <- as.numeric(((Q_CFI - dfCFI) - (Q - df))/(Q_CFI - 
                    dfCFI))
            ...
            scaleO = as.vector(lowerTriangle((S_Stand/S_LD), 
                diag = T))
            ...
            diag(V_stand2)[diag(V_stand2) == 0] <- 2e-09
            ...
            if (estimation == "ML") {
                emptystand <- .tryCatch.W.E(Fit_stand <- sem(Model1, 
                  sample.cov = S_Stand, estimator = "ML", sample.nobs = 200, 
                  std.lv = std.lv, optim.dx.tol = 0.01, sample.cov.rescale = FALSE, optim.force.converged = F))
                if (is.null(emptystand$warning$message[1])) {
                  emptystand$warning$message[1] <- 0
                }
            }
            ...
            if (class(bread_stand2$value)[1] != "matrix" | lavInspect(Fit_stand, 
                ...
                Ohtt_stand <- bread_stand %*% t(lettuce_stand) %*% 
                  Vcov_stand %*% lettuce_stand %*% bread_stand
                ...
                stand <- data.frame(inspect(Fit_stand, "list")[, 
                  c("free", "est")])
                ...
                if (exists("ghost2_stand") == "TRUE") {
                  ...
                  stand2 <- cbind(stand, SE_stand)
                }
                colnames(stand2) <- c("est_stand", "se_stand")
                df <- lavInspect(Model1_Results, "fit")["df"]
                if (!(is.character(Q))) {
                  chisq <- Q
                  AIC <- (Q + 2 * lavInspect(Model1_Results, 
                    "fit")["npar"])
                }
                else {
                  chisq <- Q
                ...
                base_model <- data.frame(inspect(ReorderModel, 
                  "list")[, c("lhs", "op", "rhs", "free", "est")])
                base_model <- subset(base_model, !(paste0(base_model$lhs, 
                  base_model$op, base_model$rhs) %in% paste0(unstand2$lhs, 
                  unstand2$op, unstand2$rhs)))
                base_model <- subset(base_model, base_model$op == 
                  "=~" | base_model$op == "~~" | base_model$op == 
                  "~")
                ...
                std_all <- subset(std_all, paste0(std_all$lhs, 
                  std_all$op, std_all$rhs) %in% paste0(results$lhs, 
                  results$op, results$rhs))
                std_all$order <- paste0(std_all$lhs, std_all$op, 
                  std_all$rhs)
                std_all <- data.frame(std_all$est.std, std_all$order)
                colnames(std_all) <- c("est.std", "order")
                results$order <- paste0(results$lhs, results$op, 
                  results$rhs)
                results$order2 <- 1:nrow(results)
                results <- suppressWarnings(merge(results, std_all, 
                  by = "order", all = T))
                results$est.std <- ifelse(is.na(results$est.std), 
                  results$est_stand, results$est.std)
    ...
    if (class(bread2$value)[1] == "matrix" & check == 2) {
        colnames(results) = c("lhs", "op", "rhs", "Unstand_Est", 
            "Unstand_SE", "STD_Genotype", "STD_Genotype_SE", 
            "STD_All")
        if (CFIcalc == TRUE) {
            colnames(modelfit) = c("chisq", "df", "AIC", "CFI", 
                "SRMR")
        }
        else {
        ...
        if (!(is.character(modelfit$chisq)) & !(is.factor(modelfit$chisq))) {
            ...
            modelfit$p_chisq <- ifelse(!(is.character(modelfit$chisq)), 
                modelfit$p_chisq <- pchisq(modelfit$chisq, modelfit$df, 
                  lower.tail = FALSE), modelfit$p_chisq <- NA)
            modelfit$chisq <- ifelse(modelfit$df == 0, modelfit$chisq == 
                NA, modelfit$chisq)
            modelfit$AIC <- ifelse(modelfit$df == 0, modelfit$AIC == 
                NA, modelfit$AIC)
            modelfit$p_chisq <- ifelse(modelfit$df == 0, modelfit$p_chisq == 
                NA, modelfit$p_chisq)
            modelfit$SRMR <- ifelse(modelfit$df == 0, modelfit$SRMR == 
                NA, modelfit$SRMR)
            if (CFIcalc == TRUE) {
                order <- c(1, 2, 6, 3, 4, 5)
                ...
                modelfit$CFI <- ifelse(modelfit$df == 0, modelfit$CFI == 
                  NA, modelfit$CFI)
            }
            else {
                order <- c(1, 2, 5, 3, 4)
        ...
        if (LD_sdiff > 0) {
            print(paste("The S matrix was smoothed prior to model estimation due to a non-positive definite matrix. The largest absolute difference in a cell between the smoothed and non-smoothed matrix was ", 
                LD_sdiff, "As a result of the smoothing, the largest Z-statistic change for the genetic covariances was ", 
                Z_diff, ". We recommend setting the smooth_check argument to true if you are going to run a multivariate GWAS.", 
                sep = " "))
        }
        if (LD_sdiff > 0.025) {
        ...
        if (LD_sdiff2 > 0) {
            print(paste("The V matrix was smoothed prior to model estimation due to a non-positive definite matrix. The largest absolute difference in a cell between the smoothed and non-smoothed matrix was ", 
                LD_sdiff2, "As a result of the smoothing, the largest Z-statistic change for the genetic covariances was ", 
                Z_diff, ". We recommend setting the smooth_check argument to true if you are going to run a multivariate GWAS.", 
                sep = " "))
        }
        ...
        results$p_value <- 2 * pnorm(abs(as.numeric(results$Unstand_Est)/as.numeric(results$Unstand_SE)), 
            lower.tail = FALSE)
        results$p_value <- ifelse(results$p_value == 0, "< 5e-300", 
            results$p_value)
        if (empty4$warning$message[1] != 0 & !grepl("not recommended for continuous data", 
            empty4$warning$message[1])) {
            warning(paste0("The unstandardized model produced the following warning: ", 
                empty4$warning$message[1], sep = ""))
        }
        if (emptystand$warning$message[1] != 0 & !grepl("not recommended for continuous data", 
            emptystand$warning$message[1])) {
            warning(paste0("The standardized model produced the following warning: ", 
                emptystand$warning$message[1], sep = ""))
        }
        if (imp_cov == FALSE) {
        ...
        if (imp_cov == TRUE) {
            ...
            names(resid_cov) <- c("Model Implied Covariance Matrix", 
                "Residual Covariance Matrix: Calculated as Observed Cov - Model Implied Cov")
            return(list(modelfit = modelfit, results = results, 
                resid_cov = resid_cov))
        }
    }
