userGWAS<-function(covstruc=NULL,SNPs=NULL,estimation="DWLS",model="",modelchi=FALSE,printwarn=TRUE,sub=FALSE,cores=NULL,toler=FALSE,SNPSE=FALSE,parallel=TRUE,GC="standard",MPI=FALSE){ 
  time<-proc.time()
        ...
        for (i in 1:f) { 
          ...
          if(resid_var2 > 0){
            ...
            if(modelchi == TRUE){
              ...
              for (g in 1:nrow(ModelQ)){
                ModelQ$free[g]<-ifelse((paste(ModelQ$lhs[g], ModelQ$op[g], ModelQ$rhs[g], sep = "") %in% modeltest2$write.test.k) & ModelQ$est[g] == 0, 1, 0)
              }
              
              ##identify components of saturated model that were already estimated in user model and label as 2
              for (g in 1:nrow(ModelQ)){
                ModelQ$free[g]<-ifelse((paste(ModelQ$lhs[g], ModelQ$op[g], ModelQ$rhs[g], sep = "") %in% modeltest2$write.test.k) & ModelQ$est[g] != 0, 2, ModelQ$free[g])
              }
              
              tester<-vector(mode="list",length=nrow(ModelQ))
              
              ##replace components of saturdated model already estimated with residual factor estimates
              for(g in 1:nrow(ModelQ)){
                if(ModelQ$free[g] == 2) { 
                  t<-paste(ModelQ$lhs[g], ModelQ$op[g], ModelQ$rhs[g], sep = "")
                  t2<-gsub("V", "VF", t)
                  tester[[g]]<-t2}else{}
              }
              ...
              for (g in 1:nrow(ModelQ)){
                ModelQ$free[g]<-ifelse((paste(ModelQ$lhs[g], ModelQ$op[g], ModelQ$rhs[g], sep = "") %in% test2), 1, ModelQ$free[g])
              }
              
              ModelQ$free<-ifelse(ModelQ$free != 1, 0, ModelQ$free)
              
              #want to freely estimate the residual factor variances and the residual covariances
              p<-length(ModelQ$free)-z
              
              ModelQ <- ModelQ[order(ModelQ$free),] 
              
              ModelQ$free <- c(rep(0, p),1:z)
              ...
              if(exists("ModelQ_Results")){
              ModelQ2 <- parTable(ModelQ_Results)
              }
              
              if(as.character(testQ$warning$message)[1] != "lavaan WARNING: model has NOT converged!" & exists("ModelQ_Results")){
              if(!(NA %in% ModelQ2$se)){
