...
addInstruments <-function(covstruc, SNPs,exposure){
  time<-proc.time()
  ...
  # flip beta's to all positively effect the variable to be instrumented
  
  beta_SNP <- beta_SNP * sign(beta_SNP[,exposure])
  ...
  # compute Variance of the polygenic instrument
  varInstrument <- sum(varSNP)
  
  # Take the product of the SNP variance and the SNP efects;
  score_adds <- beta_SNP * varSNP
  
  # compute the covariance of the polygenic instrument 
  Instrument <- rowSums(score_adds)
  ...
  for (i in 1:f) {
    ...
    #enter Instrument variance (based on MAF in reference panel) as first observation
    S_SNP[1]<- varInstrument
    
    #enter Instrument covariances 
    
      S_SNP[2:(k+1)]<- Instrument
    ...
    for (p in 1:nrow(coords)) { 
      ...
      y<-coords[p,2]
      if (x != y) { 
        V_SNP[x,y]<-(sqrt(sum(SE_SNP[,y]^2))*sqrt(sum(SE_SNP[,x]^2))*I_LD[x,y]*I_LD[x,x]*I_LD[y,y]*varInstrument^2)}
      if (x == y) {
        V_SNP[x,x]<-(sqrt(sum(SE_SNP[,x]^2))*I_LD[x,x]*varInstrument)^2
      }
    }
