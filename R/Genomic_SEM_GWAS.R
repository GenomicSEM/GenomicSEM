...
##example files provided for first 500 SNPs for p-factor
S_LD<-read.csv("S_LD.csv",header=FALSE)
V_LD<-read.csv("V_LD.csv",header=FALSE)
I_LD<-read.csv("I_LD.csv",header=FALSE)
beta_SNP<-read.csv("beta_SNP.csv",header=FALSE)
SE_SNP<-read.csv("SE_SNP.csv",header=FALSE)
MAF<-read.csv("MAF.csv",header=FALSE)

Test<-Genomic_SEM_GWAS(S_LD,V_LD,I_LD,beta_SNP,SE_SNP,MAF)

###the actual function. Currently it will create a 1 factor model and run 
##the GWAS. Need to update with user options for what they are saving from the runs
##and user options for the type of model specified, with some standard models pre-packaged

Genomic_SEM_GWAS<-function(S_LD, V_LD, I_LD, beta_SNP, SE_SNP, MAF){
