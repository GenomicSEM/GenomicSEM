...
munge <- function(files,hm3,trait.names=NULL,N,info.filter = .9,maf.filter=0.01){
  ...
  for(i in 1:length){
    ...
    hold_names[hold_names %in%c("P","p","PVALUE","Pval","pvalue","P_VALUE","P_value","P-value","p-value","`P-value`","p_value","PVAL","pval","P_VAL","p_val","GC_PVALUE","gc_pvalue", "P_Value")] <- "P"
    ...
    hold_names[hold_names %in%c("MAF","maf", "CEUaf", "Freq1", "EAF", "Freq1.Hapmap", "FreqAllele1HapMapCEU", "Freq.Allele1.HapMapCEU", "EFFECT_ALLELE_FREQ")] <- "MAF"
