...
munge <- function(files,hm3,trait.names=NULL,N,info.filter = .9,maf.filter=0.01){
  ...
  files = lapply(files, read_table, header=T, quote="\"",fill=T,na.string=c(".","NA",""))
  ...
  for(i in 1:length){
    ...
    hold_names[hold_names %in%c("P","p","PVALUE","Pval","pvalue","P_VALUE","P_value","P-value","p-value","`P-value`","p_value","PVAL","pval","P_VAL","p_val","GC_PVALUE","gc_pvalue" )] <- "P"
    ...
    if(sum(hold_names %in% "effect") == 0) warning(paste0('Cannot rs-id column, try renaming it SNP in the summary statistics file for:',filenames[i]))
