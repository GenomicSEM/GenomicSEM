ldsc <- function(traits, sample.prev, population.prev, ld, wld,
                ...
                n.blocks = 200, ldsc.log = NULL, stand = FALSE,select=FALSE,chisq.max = NA,int.est=TRUE,ct.int.est=TRUE,int.est.mat=NULL,int.est.val=NULL) {
  ...
  if(!is.null(int.est.mat) & is.matrix(int.est.mat) & is.logical(int.est.mat)){
    .LOG("You have provided what appears to be a valid intercept constrained matrix, overiding `int.est` and `ct.int.est`" , file=log.file) 
    int.est=FALSE
    ct.int.est=FALSE
  }
  ...
  if(!is.null(int.est.mat) & (!is.matrix(int.est.mat) | !is.logical(int.est.mat))){
    stop("the intercept constrained matrix does not appear to be a matrix with logical values, specify a correct intercept constrained matrix or set the argument to NULL")
  }
  if(!is.null(int.est.val) & (!is.matrix(int.est.val) | !is.numeric(int.est.val))){
    stop("the intercept constrained values do not appear to be a matrix or numeric, specify a correct intercept constrained values or set the argument to NULL")
  }
  ...
  for(j in 1:n.traits){
    ...
    for(k in j:length(traits)){
      ...
      if(j == k){
        ...
        # if the user specified an intercept constrained matrix, overrule the global argument
        if(!is.null(int.est.mat)){
        int.est= int.est.mat[j,k] 
        }
        ...
        if(int.est==FALSE){
          ...
          weighted.chi <- as.matrix((merged$chi1-int.est.val[j,k]) *merged$weights)
        }
        ...
        for(i in 1:n.blocks){
          if(int.est==TRUE){xty.block.values[i,] <- t(t(weighted.LD[select.from[i]:select.to[i],])%*% weighted.chi[select.from[i]:select.to[i],])
          ...
          if(int.est==FALSE){xty.block.values[i,] <- t(t(weighted.LD[select.from[i]:select.to[i]])%*% weighted.chi[select.from[i]:select.to[i]])
                            xtx.block.values[i,] <- (t(weighted.LD[select.from[i]:select.to[i]])%*% weighted.LD[select.from[i]:select.to[i]])}
        ...
        for(i in 1:nrow(xtx)){
          if(int.est==TRUE){xtx[i,] <- t(colSums(xtx.block.values[seq(from=i,to=nrow(xtx.block.values),by=ncol(weighted.LD)),]))}
          if(int.est==FALSE){xtx <- sum(xtx.block.values[seq(from=i,to=nrow(xtx.block.values),by=ncol(weighted.LD))])}
        }
        ...
        delete.values <- matrix(data=NA,nrow=n.blocks,ncol =(n.annot+inte))
      ...
      if(j != k){
        ...
        if(ct.int.est==FALSE){
          weighted.LD <- as.matrix(merged$L2*merged$weights)
          weighted.chi <- as.matrix((merged$ZZ - int.est.val[j,k]) *merged$weights_cov)
        }
        ...
        n.annot <- 1
        if(ct.int.est==TRUE){ inte <- 1}
        if(ct.int.est==FALSE){ inte <- 0}
        ...
        for(i in 1:n.blocks){     
          if(ct.int.est==TRUE){xty.block.values[i,] <- t(t(weighted.LD[select.from[i]:select.to[i],])%*% weighted.chi[select.from[i]:select.to[i],])
        xtx.block.values[replace.from[i]:replace.to[i],] <- as.matrix(t(weighted.LD[select.from[i]:select.to[i],])%*% weighted.LD[select.from[i]:select.to[i],])}
          if(ct.int.est==FALSE){xty.block.values[i,] <- t(t(weighted.LD[select.from[i]:select.to[i]])%*% weighted.chi[select.from[i]:select.to[i]])
          xtx.block.values[i,] <- (t(weighted.LD[select.from[i]:select.to[i]])%*% weighted.LD[select.from[i]:select.to[i]])}
        }
        xty <- as.matrix(colSums(xty.block.values))
        xtx <- matrix(data=NA,nrow =(n.annot+inte),ncol =(n.annot+inte))
        colnames(xtx)<- colnames(weighted.LD)
        for(i in 1:nrow(xtx)){
          if(ct.int.est==TRUE){xtx[i,] <- t(colSums(xtx.block.values[seq(from=i,to=nrow(xtx.block.values),by=ncol(weighted.LD)),]))}
          if(ct.int.est==FALSE){xtx <- sum(xtx.block.values[seq(from=i,to=nrow(xtx.block.values),by=ncol(weighted.LD))])}
          }
  ...
  if(int.est == FALSE ||ct.int.est == FALSE || !is.null(int.est.mat)){
    warning("You have constrained some or all of the  LD score intercept, this is risky unless you have strong prior knowledge about the (absence of) populations stratification and/or sample overlap betwene the GWASs")
